...
type GlyphProps = { tone: PillarTone };

function OperationsGlyph({ tone }: GlyphProps) {
  const rows = [16, 40, 64];
  return (
    <>
      {rows.map((y, i) => (
...
function GrowthGlyph({ tone }: GlyphProps) {
...
function InfrastructureGlyph({ tone }: GlyphProps) {
  const nodes = [
...
const glyphs: Record<PillarId, ComponentType<GlyphProps>> = {
...
type PillarGlyphProps = {
  id: PillarId;
  /** Run the draw-in on its own when visible, instead of waiting for a parent variant. */
  standalone?: boolean;
};
